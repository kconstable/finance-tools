...
def get_amortization(start_date,price,deposit,payment,yrs,int_rate,app_rate,frequency,re_fees):
    """
    Creates an amortization schedule for a mortgage repayment. Assumes monthly
    or bi-weekly (15th, last of month) payments. Adjust paymnet according to 
    the frequncy. Interest rates are converted to monthly or bi-weeky, and 
    charged at month-end or the 15th and month-end after payments have been
    applied. Creditors normally accrue interest daily.  This simplificaiton
    ...
    deposit : float (percent *100)
        Percentage of the downpayment (20% = 20.0)
    int_rate : float (perent *100)
    ...
    payment: float
        A payment amount- should match the frequency
    frequency: string
...
def plot_amortization(df_amort,yrs=[5,10]):
    ...
    for y in yrs:
        if freq=='m':
            p = y * 12
            try:
                d[y]={'date':df.at[p,'date'],'mtg':df.at[p,'end'],'name':f'After {y} Years'}
            except:
                continue
        else:
            p = y *24
...
def get_rent_vs_own(start_date,price,deposit,payment,yrs,int_rate,app_rate,frequency,re_fees,monthly_rent,inv_rate,monthly_main,annual_tax):
    
    
    # get the mortgage amortization schedule
    df = get_amortization(start_date, price, deposit, payment, yrs, int_rate, app_rate, frequency,re_fees)
    ...
    # calculte the investment returns
    cross_over = True
    for idx, row in df.iterrows():
        # first period. invest the downpayment + tax + main + mortgage/rent difference
        if idx == 0:
            invest = downpayment + tax + main + (payment-rent)
...
# df = get_amortization(START_DATE, PRICE, DEPOSIT, PAY, YRS, IR, APP_RATE, 'm',RE_FEES)
df = get_rent_vs_own(START_DATE, PRICE, DEPOSIT, PAY, YRS, IR, APP_RATE, 'b',RE_FEES, MONTHLY_RENT, ANNUAL_INVEST_RATE, MONTHLY_FEES, ANNUAL_TAX)
plot_rent_vs_own(df)
